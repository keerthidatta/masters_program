...
class SteeringLimits: # should be the encoder values at the limits. if they change with resets, we need to calibrate them first. shouldn't be a biggie
	...
	STEERING_FRONT_LEFT = [195, 1370]
	STEERING_FRONT_RIGHT = [225, 1381]
	STEERING_BACK_LEFT = [340, 1280] #TODO measure those constants
	STEERING_BACK_RIGHT = [213, 1345] #TODO measure those constants
	CAM_TILT = [638, 882] #TODO measure those constants
...
def ConvertMeterToTicks(meter):
	return meter / (wheel_diameter * 3.1415926535897 / ticks_per_rotation)

def ConvertMperSToEncSpeed(mpers):
	return ConvertMeterToTicks(mpers)

def ConvertTicksToMeters(ticks):
	return float(ticks) / ticks_per_rotation * (wheel_diameter * 3.1415926535897)
...
def ConvertAbsEncToRad(enc_val, limits):
	...
	#if limits == SteeringLimits.STEERING_FRONT_LEFT:
	#	print "Limits: " + str(limits)
	#	print "offset: " + str(offset)
	#	print "enc range: " + str(enc_range)
	#	print "enc val: " + str(enc_val)
	#	print "steering_angle_range: " + str(steering_angle_range)
	return float(enc_val - offset) / enc_range * steering_angle_range
...
def publishValidLinVel(pub, motor_sel):
	...
	if enc_val[0] == 1:
		...
		#if motor_sel == MotorSelect.WHEEL_FRONT_LEFT:
		#	print "Wheel front left"
		#	print w.linear.x
		#	print enc_val[1]
		pub.publish(w)
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		#leftwheel.linear.x = 0.2
		#centerleftwheel.linear.x = 0.2
		#rearleftwheel.linear.x = 0.2
		#rightwheel.linear.x = 0.2
		#centerrightwheel.linear.x = 0.2
		#rearrightwheel.linear.x = 0.2
		MoveRover(leftwheel,centerleftwheel, rearleftwheel, rightwheel, centerrightwheel,rearrightwheel)
		

		#ReadEncodersTest()
		PublishEncValues(pub1, pub2, pub3, pub4, pub5, pub6)
		#print speeds
		#StopRover()
